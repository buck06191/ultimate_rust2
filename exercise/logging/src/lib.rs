...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        if self.energy == 0 {
            ...
            warn!(target: "Frog::hop", "Frog ran out of energy and is going to sleep");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target:"Frog::sleep", "Frog already asleep");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Fog::default", "Default value generated: {:?}", default_frog);
        default_frog
    }
}
